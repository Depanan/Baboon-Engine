...
VulkanBuffer::VulkanBuffer(Device& device,
    VkDeviceSize             size,
...
void VulkanBuffer::update(const std::vector<uint8_t>& data, size_t offset)
{
    update(data.data(), data.size(), offset);
}

void VulkanBuffer::update(void* data, size_t size, size_t offset)
{
    update(reinterpret_cast<const uint8_t*>(data), size, offset);
}

void VulkanBuffer::update(const uint8_t* data, const size_t size, const size_t offset)
{
    if (m_Persistent)
    {
